...
def main():
    ...
    layers = [p for p in model.modules() if hasattr(p, "_hybrid_swiglu_calls")]
    for _ in range(4):
    ...
    try:
        for group in range(args.groups):
            for mixed in (
                ...
                hybrid_calls = sum(p._hybrid_swiglu_calls for p in layers) - before
                assert hybrid_calls == (28 if mixed else 0)
